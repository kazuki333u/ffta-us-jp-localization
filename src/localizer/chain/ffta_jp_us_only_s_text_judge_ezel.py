#! python3
"""Editorial localization of the US-only s_text judge-corruption arc and the
Ezel mediator scene.

Fifth *class 5* milestone, second s_text editorial batch.  Five whole US
scene-dialogue leaves that exist only in the US ROM:

* s_text top 47 (52 lines) -- judge-corruption 1: Cid and the corrupt judge, Cid recruit
* s_text top 51 (33 lines) -- judge-corruption 2: the Cyril judge and the seals of immunity
* s_text top 52 (27 lines) -- judge-corruption 3: the Baguba judge and the "donations"
* s_text top 53 (47 lines) -- judge-corruption 4: the Sprohm assembly, the prison judge
* s_text top 55 (65 lines) -- Ezel mediates the resistance/palace talks, Ezel recruit

224 lines: 219 of the exact 776-entry editorial scope plus the five line-0
scene openers (``SCENE_COMPLETION``, same reasoning as the special-scenes
layer: the aligner pairs each line 0 with the JP page-level repeat marker, so
those lines have no JP original either and the scenes cannot ship with their
opening line in English).

The translations are NOT computed here.  They are read from the tracked
manifest ``data/us_only_s_text_judge_ezel.json``, the editorial decision of
record; this module encodes and installs it, and revalidates it against both
ROMs on every build.  The mechanism is identical to
``ffta_jp_us_only_s_text`` (whole-leaf serialize + root repoint, tail
relocation after that layer's block); this module builds on that layer's
output and is the new terminal layer, so its EXPECTED_PRODUCTION is the single
canonical final-SHA authority.
...
import ffta_jp_us_only_s_text as prev
...
MANIFEST = HERE / "data/us_only_s_text_judge_ezel.json"
RUN_BASE = HERE / "build/us_only_s_text_localization"
OUTROM = ROOT / "rom/build/ffta_us_jp_s_text_judge_ezel.gba"
OUTROM2 = ROOT / "rom/build/ffta_us_jp_s_text_judge_ezel_repeat.gba"

# Output of ffta_jp_us_only_s_text, the previous production layer.  That
# module's EXPECTED_PRODUCTION is now a private per-layer drift gate.
BASELINE = "1A37D0E231172AA027BF169C1260672C0AAAC058E0F61F771228B842FC08142C"
...
EXPECTED_PRODUCTION = "A6C533D06EE9EB237860F410A1B0FDF949257F0B9CF5EC3CDD3B5F1D055CBE9D"

S_TEXT_TABLE_POINTER = 0x009A88
TARGET_TOPS = (47, 51, 52, 53, 55)
TOP_TSIZE = {47: 52, 51: 33, 52: 27, 53: 47, 55: 65}
TOTAL = 224
IN_SCOPE = 219
SCENE_COMPLETION = 5
...
def jp_entry(jp, path):
    """Resolve a JP logical path to its token list; unlike
    ``sysbat.jp_entry`` this also accepts the nested ``s_text/61/<sub>/<line>``
    paths of the JP battle-dialogue table, where a few kanji are attested."""
    parts = path.split("/")
    if path.startswith(("pages:", "words:")) or len(parts) == 3:
        return sysbat.jp_entry(jp, path)
    if len(parts) != 4 or parts[0] != "s_text":
        raise RuntimeError(f"SJE_ATTESTATION_PATH_SHAPE {path}")
    line = jp.tabs["s_text"][int(parts[1])][int(parts[2])][int(parts[3])]
    text = getattr(line, "text", line)
    return list(getattr(text, "tokens", []) or [])
...
def kanji_table(doc, jp):
    ...
    for char, row in doc["kanji_codes"].items():
        ...
        if code < 0x122:
            raise RuntimeError(f"SJE_KANJI_CODE_NOT_IN_KANJI_SPAN {char}")
        path = row["attested_at"]
        if not path:
            raise RuntimeError(f"SJE_KANJI_ATTESTATION_MISSING {char}")
        if not any(kind == "CHR_FULL" and value == code
                   for kind, value in jp_entry(jp, path)):
            raise RuntimeError(f"SJE_KANJI_ATTESTATION_FAILED {char} "
                               f"0x{code:04X} {path}")
        if char in table:
            raise RuntimeError(f"SJE_KANJI_DUPLICATE {char}")
        table[char] = code
    return table
...
def load_manifest(jp, us, decode):
    ...
    if len(rows) != TOTAL or doc["count"] != TOTAL:
        raise RuntimeError(f"SJE_MANIFEST_COUNT {len(rows)}/{doc['count']}")
    if doc["baseline_production_sha256"] != BASELINE:
        raise RuntimeError("SJE_MANIFEST_BASELINE_DRIFT")
    if (doc["translated_count"] != TOTAL or doc["no_change_required_count"] != 0
        ...
        raise RuntimeError("SJE_MANIFEST_STATUS_COUNTS")
    ...
    owned_elsewhere = (set(stext.EXPECTED_TOPS) | set(recovered.TARGET_TOPS)
                       | set(prev.TARGET_TOPS))
    if owned_elsewhere & set(TARGET_TOPS):
        raise RuntimeError("SJE_TOP_OWNED_BY_ANOTHER_LAYER")
    ...
    for row in rows:
        ...
        if row["us_logical_path"] != f"s_text/{top}/{line}":
            raise RuntimeError(f"SJE_MANIFEST_PATH {row['us_logical_path']}")
        if row["status"] != "TRANSLATED_REVIEWED":
            raise RuntimeError(f"SJE_MANIFEST_STATUS {row['us_logical_path']}")
        if row["scope"] not in ("EDITORIAL_776", "SCENE_COMPLETION"):
            raise RuntimeError(f"SJE_MANIFEST_SCOPE {row['us_logical_path']}")
        if (row["scope"] == "SCENE_COMPLETION") != (line == 0):
            raise RuntimeError(f"SJE_SCOPE_LINE0_MISMATCH {row['us_logical_path']}")
        if not row["japanese"]:
            raise RuntimeError(f"SJE_MANIFEST_EMPTY {row['us_logical_path']}")
        page = root[top]
        if not isinstance(page, c_ffta_sect_text_page):
            raise RuntimeError(f"SJE_TARGET_NOT_A_LEAF {top}")
        if page.tsize != TOP_TSIZE[top]:
            raise RuntimeError(f"SJE_US_LEAF_TSIZE {top} {page.tsize}")
        if not _us_source.matches(row, visible(tokens_of(page[line]), decode)):
            raise RuntimeError(f"SJE_MANIFEST_ENGLISH_DRIFT {row['us_logical_path']}")
        state = status.get((top, line), "NO_JP_ENTRY_ALIGNED")
        if state == "HAS_JP":
            raise RuntimeError(f"SJE_TARGET_HAS_JP_ORIGINAL {row['us_logical_path']}")
        if line == 0 and state != "JP_REPEAT_PAGE_MARKER":
            raise RuntimeError(f"SJE_LINE0_REASON {row['us_logical_path']} {state}")
        row["_us_only_reason"] = state
    if {k: sorted(v) for k, v in got.items()} != \
            {t: list(range(TOP_TSIZE[t])) for t in TARGET_TOPS}:
        raise RuntimeError(f"SJE_MANIFEST_NOT_WHOLE_PAGES {sorted(got)}")
    return doc, rows
...
def parse_markup(japanese, reverse, kanji):
    ...
    while i < len(japanese):
        ...
        if char in kanji:
            ...
            raise RuntimeError(f"SJE_UNENCODABLE {char!r} in {japanese!r}")
        i += 1
    return out
...
def family_bounds(jp, jp_raw):
    ...
    for top in BOUND_TOPS:
        page = root[top]
        if not isinstance(page, c_ffta_sect_text_page):
            raise RuntimeError(f"SJE_BOUND_PAGE_NOT_A_LEAF {top}")
    ...
    if not width or not lines:
        raise RuntimeError("SJE_BOUND_EMPTY")
    ...
    for kind, value in tokens:
        if kind == "CHR_FULL":
            if value not in alloc:
                raise RuntimeError(f"SJE_GLYPH_NOT_ALLOCATED 0x{value:04X}")
...
def encode(row, reverse, kanji, alloc, jp_raw, bounds, original):
    ...
    if probe.tokens != expected:
        raise RuntimeError(f"SJE_SERIALIZER_ROUNDTRIP_FAILED {path}")
    if data[-1] != 0:
        raise RuntimeError(f"SJE_EOS_MISSING {path}")
...
def product_leaf(data, root, top):
    ...
    while True:
        ...
        count += 1
        if count > 4096:
            raise RuntimeError(f"SJE_PRODUCT_LEAF_UNTERMINATED {top}")
...
def product_leaf_tokens(data, root, top, expected_tsize):
    ...
    if count != expected_tsize:
        raise RuntimeError(f"SJE_PRODUCT_LEAF_TSIZE {top} {count}")
...
def build():
    ...
    if int.from_bytes(us_raw[S_TEXT_TABLE_POINTER:S_TEXT_TABLE_POINTER + 4],
        ...
        raise RuntimeError("SJE_ROOT_DECL_DRIFT")
    ...
    for top in TARGET_TOPS:
        ...
        if sharers:
            raise RuntimeError(f"SJE_TARGET_LEAF_SHARED {top} {sharers}")
    ...
    for top in TARGET_TOPS:
        ...
        if len(replacements[top]) != page.tsize:
            raise RuntimeError(f"SJE_LEAF_REPLACEMENT_COUNT {top}")
...
def validate(product, base, meta, alloc, records, rows, leaf_records,
    ...
    if sha(OUTROM) != sha(product):
        raise RuntimeError("SJE_READBACK_ROM_MISMATCH")
    ...
    if check_root != live_root:
        raise RuntimeError("SJE_READBACK_ROOT_OFFSET")
    ...
    for top in TARGET_TOPS:
        ...
        if rel != expected_relative:
            raise RuntimeError(f"SJE_READBACK_ROOT_POINTER {top}")
    ...
    for row, record in zip(rows, records):
        ...
        got = render(tokens)
        if got != row["japanese"]:
            raise RuntimeError(f"SJE_READBACK_TEXT_MISMATCH "
                               f"{row['us_logical_path']} {got!r}")
        if tokens != record["_expected"]:
            raise RuntimeError(f"SJE_READBACK_TOKEN_MISMATCH "
    ...
    for top in TARGET_TOPS:
        ...
        for index in range(TOP_TSIZE[top]):
            if tokens[index] != expectations[top][index]:
                raise RuntimeError(f"SJE_LEAF_TOKEN_MISMATCH {top}/{index}")
    root_tsize = 63
    ...
    if moved:
        raise RuntimeError(f"SJE_SIBLING_ROOT_MOVED {moved}")
    ...
    if len(set(ours)) != len(ours):
        raise RuntimeError("SJE_POINTER_COLLISION")
    foreign = {relatives[t] for t in range(root_tsize) if t not in TARGET_TOPS}
    if set(ours) & foreign:
        raise RuntimeError("SJE_ALIAS_PROPAGATION")
    for record in leaf_records:
        ...
        if any(lo <= relatives[t] < hi for t in range(root_tsize)
            ...
            raise RuntimeError(f"SJE_ALIAS_HAZARD {record['us_top']}")
    audits["sibling"] = {
    ...
    if product[fx_span[0]:fx_span[1]] != base[fx_span[0]:fx_span[1]]:
        raise RuntimeError("SJE_FX_TEXT_ROOT_DISTURBED")
    families = {}
    for name, pool in us.tabs["words"].items():
        ...
        if product[span[0]:span[1]] != base[span[0]:span[1]]:
            raise RuntimeError(f"SJE_WORDS_FAMILY_DISTURBED words:{name}")
    ...
    if product[sysbat.PAGES_ROOT_FIELD:sysbat.PAGES_ROOT_FIELD + 4] != \
       base[sysbat.PAGES_ROOT_FIELD:sysbat.PAGES_ROOT_FIELD + 4]:
        raise RuntimeError("SJE_PAGES_BATTLE_DISTURBED")
    ...
    if stray:
        raise RuntimeError(f"SJE_SLOT_NOT_FROM_JP_ALLOCATION {stray[:8]}")
    jp_raw = JP.read_bytes()
    for slot in used:
        code = inverse[slot]
        ...
        if product[uoff:uoff + stext.FONT_STRIDE] != \
            ...
            raise RuntimeError(f"SJE_SLOT_NOT_JP_IDENTICAL 0x{slot:04X}")
    ...
    if unexplained:
        raise RuntimeError(f"SJE_BINARY_TOUCH_REGRESSION {unexplained[:8]}")
    ...
    if code_ranges:
        raise RuntimeError(f"SJE_CODE_TOUCHED {code_ranges[:4]}")
    audits["binary"] = {
...
def main():
    ...
    ap.add_argument("--run", default="20260830_judge_ezel_production")
    ...
    if sha(product) != sha(second[0]) or strip(records) != strip(second[4]):
        raise RuntimeError("SJE_BUILD_NONDETERMINISTIC")
    if (bs, be) != (second[10], second[11]) or leaf_records != second[7]:
        raise RuntimeError("SJE_LAYOUT_NONDETERMINISTIC")
    ...
    summary = {
        ...
        "batch": {"entries": len(records), "translated": len(records),
                  "in_editorial_776_scope": sum(
                      1 for r in records if r["scope"] == "EDITORIAL_776"),
        ...
        "coverage": {
            ...
            "class_5_editorial_produced_before": 993,
            "class_5_translated_by_this_layer_in_776_scope": sum(
                1 for r in records if r["scope"] == "EDITORIAL_776"),
            "class_5_scene_completion_outside_776_scope": sum(
                1 for r in records if r["scope"] == "SCENE_COMPLETION"),
