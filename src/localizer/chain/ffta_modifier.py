...
CONF = {
    ...
    'text': {
        ...
        'align': {
            's_text': [
                # Story leaves where the US ROM inserts new dialogue *before*
                # the trailing developer cut-index labels: the JP labels sit at
                # the JP leaf's tail and must follow the US insertion, or the
                # raw-index pairing writes a JP "カット番号" label over each
                # inserted English line (hidden English) and leaves the US
                # copies of the labels unpaired.  Read off both leaves:
                # top 1: JP 118..128 = 11 labels, US 118..170 = 53 snowball-
                #   tutorial / Randell lines, US 171..181 = the same 11 labels.
                # top 19: JP 101..109 = 9 labels, US 101..106 = 6 Mewt/Cid
                #   apology lines, US 107..115 = the same 9 labels.
                # top 26: JP 71..76 = 6 labels, US 71 = one rewritten Mewt
                #   line (scene FAT 77 reads it), US 72..77 = empty records.
                ((1, 118), (1, 171)),
                ((19, 101), (19, 107)),
                ((26, 71), (26, 72)),
                ((36,), (35,)),
                ((60,), (60,)),
                # Top 61 is the mission-battle dialogue table (26 sub-pages).
                # Both ROMs hold the same missions in the same order, but a
                # mission block is 15 lines in JP (intro, 6 race variants,
                # unused, defeat, 6 race variants) and 24 lines in US (the
                # same 15 plus 9 unused slots), so JP 61/s/15b+k is
                # US 61/s/24b+k.  Without these anchors the aligner pairs by
                # raw index and, from block 1 on, writes each JP line into the
                # wrong US slot (or an empty JP record over US text).  Sub 25
                # holds five blocks (JP 75 / US 120 lines), the others ten.
                *[((61, sub, 15 * block), (61, sub, 24 * block))
                  for sub in range(26)
                  for block in range(1, 10 if sub < 25 else 5)],
            ],
            'pages:battle': [
                # JP 51..53 are three "Holy aura heals HP!" lines; US inserts
                # three judge-system lines at 51..53, so JP 51 aligns to US 54.
                ((51,), (54,)),
            ],
            'pages:quest/': [
                ((1, 0), (0, 200)),
            ...
            'fx_text': [
                # US inserts two rumours at 8/58 ([Unfair Judges?]) and 8/59
                # ([Blank Cards]), shifting the tutorial block by two: JP 8/58
                # ("! アビリティ") is US 8/60 ("! Abilities !") and JP 8/60
                # ("! アビリティの種類") is US 8/61 ("! Ability Types !").
                # JP 8/59 and 8/61 are empty records that absorb the same two
                # slots on the JP side, which is why both leaves hold 91
                # entries and why 8/62 realigns with no further offset.
                ((8, 58), (8, 60)),
                ((8, 60), (8, 61)),
                ((8, 62), (8, 62)),
                ((25,), (24,)),
            ...
            'words:rumor': [
                # US inserts two rumours of its own -- "Unfair Judges?" (59)
                # and "Blank Cards" (60) -- ahead of the tutorial block and
                # then packs the tutorial titles from 61 on.  JP keeps those
                # tutorial titles on the odd indices 59/61/63... with numeric
                # placeholders ("060", "062") in the even slots between them,
                # so JP 59 is the original of US 61 and JP 61 of US 62.
                # The earlier single anchor ((62,), (61,)) paired the
                # placeholders with the shipped list -- runtime showed "062"
                # as the title of the abilities tutorial -- and pushed JP 59
                # onto the US-only rumour.  US 59 and 60 have no JP original
                # at all; they are localized editorially by
                # ffta_jp_us_only_system_battle.
                ((59,), (61,)),
                ((61,), (62,)),
                ((63,), (63,)),
            ],
            'words:battle': [
                # US inserts three law entries at 176..178, so JP 176
                # ("MPすりかえ" = "Damage > MP") aligns to US 179.
                ((176,), (179,)),
                # US inserts nine more judge/law entries at 534..542, so the
                # JP combo block starting at 531 aligns to US 543 ("Blue Combo").
                ((531,), (543,)),
            ],
        },
        'trim': {
